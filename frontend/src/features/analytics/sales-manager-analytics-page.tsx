...
function leadHref(data: ManagerAnalytics | null, extra: Record<string, string> = {}) {
...
export function SalesManagerAnalyticsPage() {
  ...
  return <section className="page manager-page">
    ...
    {loading && !data ? <div className="panel sales-analytics-loading">Loading branch analytics...</div> : data && <>
      ...
      {activeTab === "overview" && <section className="manager-overview-grid"><article className="panel manager-funnel"><header className="manager-card-head"><div><p className="eyebrow">FUNNEL LEAKAGE</p><h2>Lead to retail path</h2></div><Link className="filter" href={leadHref(data)}>Open leads</Link></header>{data.funnel.map(item => <div className="manager-funnel-row" key={item.key}><span>{item.label}</span><div><i style={{ width: `${Math.max(3, (item.count / maxFunnel) * 100)}%` }} /></div><b>{item.count}</b><small>{item.rate}%</small></div>)}</article><article className="panel manager-risk-card"><p className="eyebrow">OPS RISKS</p><h2>Needs attention</h2><div className="manager-risk-list"><Link href={leadHref(data, { status: "FRESH" })}><b>{data.summary.stale_untouched}</b><span>stale untouched leads</span></Link><Link href={leadHref(data)}><b>{data.followups.overdue}</b><span>overdue follow-ups</span></Link><Link href={leadHref(data)}><b>{data.summary.flagged}</b><span>flagged to manager</span></Link><Link href={leadHref(data, { status: "LOST" })}><b>{data.summary.lost}</b><span>lost or unqualified</span></Link></div></article></section>}
      {activeTab === "cre" && <RoleTable title="CRE performance" section="cre" rows={data.cre} data={data} onExport={exportSection} />}
      {activeTab === "ps" && <RoleTable title="PS/SO performance" section="ps" rows={data.ps} data={data} onExport={exportSection} />}
      {activeTab === "source" && <section className="manager-two-col"><PerformanceTable title="Source conversion" section="source" labelKey="source" rows={data.source} data={data} onExport={exportSection} /><PerformanceTable title="Model conversion" section="models" labelKey="model" rows={data.models} data={data} onExport={exportSection} /></section>}
      {activeTab === "ops" && <section className="manager-two-col"><article className="panel manager-table-card"><header className="manager-card-head"><div><p className="eyebrow">LOST ANALYSIS</p><h2>Lost outcomes</h2></div><button className="filter" onClick={() => exportSection("lost_reasons")}>Export CSV</button></header><div className="manager-list">{data.lost_reasons.length ? data.lost_reasons.map(row => <Link href={leadHref(data, { status: "LOST" })} key={row.outcome}><span>{row.outcome}</span><b>{row.count}</b></Link>) : <p className="subtext">No structured lost outcomes for this period.</p>}</div></article><article className="panel manager-table-card"><header className="manager-card-head"><div><p className="eyebrow">STALE LEADS</p><h2>Oldest untouched</h2></div><button className="filter" onClick={() => exportSection("stale_leads")}>Export CSV</button></header><div className="manager-list">{data.stale_leads.length ? data.stale_leads.map(row => <Link href={`/manager/leads?q=${encodeURIComponent(row.phone)}`} key={row.id}><span>{row.name}<small>{sourceName(row.source)} - {row.model_interest || "Model not set"} - {formatDate(row.created_at)}</small></span><b>#{String(row.id).padStart(6, "0")}</b></Link>) : <p className="subtext">No stale untouched leads.</p>}</div></article></section>}
    </>}
  </section>;
}
