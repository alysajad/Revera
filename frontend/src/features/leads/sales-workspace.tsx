...
export function SalesWorkspace({ followUpsOnly = false }: { followUpsOnly?: boolean }) {
  ...
  const currentStatusOptions = useMemo(() => {
    if (!detail) return [];
    const currentStatus = detail.nextFollowUp && ["FRESH", "RNR"].includes(detail.statusCode) ? "CALLBACK" : detail.statusCode;
    return statusOptions[currentStatus] || [];
  }, [detail]);
  const selectStatus = (status: string) => setDraft(current => current ? { ...current, status, follow_up_at: followUpStatuses.has(status) ? current.follow_up_at : "" } : current);
