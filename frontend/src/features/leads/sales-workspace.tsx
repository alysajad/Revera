...
export function SalesWorkspace({ followUpsOnly = false }: { followUpsOnly?: boolean }) {
  ...
  const selectCallOutcome = (call_outcome: string) => setDraft(current => current ? { ...current, call_outcome, status: outcomeStatuses[call_outcome] || current.status, sales_outcome: call_outcome === "LOST" ? "LOST" : current.sales_outcome, follow_up_at: call_outcome === "PENDING" ? current.follow_up_at : "" } : current);
