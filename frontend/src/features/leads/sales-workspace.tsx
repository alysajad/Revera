...
const outcomeLabels: Record<string, string> = { QUALIFIED: "Qualified", NOT_CONNECTED: "Not connected", PENDING: "Pending", LOST: "Lost" };
const outcomeStatuses: Record<string, string> = { QUALIFIED: "QUALIFIED", NOT_CONNECTED: "RNR", PENDING: "CALLBACK", LOST: "LOST" };
...
export function SalesWorkspace({ followUpsOnly = false }: { followUpsOnly?: boolean }) {
  ...
  const save = async () => {
    ...
    try {
      const updated = await updateMyLead(detail.id, { ...draft, qualification: draft.call_outcome === "QUALIFIED" ? draft.qualification : undefined, follow_up_at: followUp ? followUp.toISOString() : null });
  ...
  const selectCallOutcome = (call_outcome: string) => setDraft(current => current ? { ...current, call_outcome, status: outcomeStatuses[call_outcome] || current.status, sales_outcome: call_outcome === "LOST" ? "LOST" : current.sales_outcome, follow_up_at: call_outcome === "PENDING" ? current.follow_up_at || minimumFollowUpDate() : "" } : current);
