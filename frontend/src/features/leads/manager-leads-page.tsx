...
export function ManagerLeadsPage({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  return <section className="page manager-page manager-leads-page">
    ...
    <section className="panel manager-filter-bar manager-lead-filters"><label>Search<input value={filters.q} onChange={event => setFilter("q", event.target.value)} placeholder="Name, phone, model" /></label><label>Source<select value={filters.source} onChange={event => setFilter("source", event.target.value)}><option value="">All sources</option>{["META", "WEBSITE", "CARWALE", "WALKIN", "CAMPAIGN", "OTHER", "UNKNOWN"].map(item => <option value={item} key={item}>{sourceName(item)}</option>)}</select></label><label>Status<select value={filters.status} onChange={event => setFilter("status", event.target.value)}><option value="">All statuses</option>{["FRESH", "RNR", "SWITCHED_OFF", "CALLBACK", "PENDING", "QUALIFIED", "WALKIN", "WON", "LOST", "UNQUALIFIED"].map(item => <option value={item} key={item}>{statusName(item)}</option>)}</select></label><label>Category<select value={filters.category} onChange={event => setFilter("category", event.target.value)}><option value="">All categories</option><option value="HOT">Hot</option><option value="WARM">Warm</option><option value="COLD">Cold</option></select></label><label>Model<select value={filters.model} onChange={event => setFilter("model", event.target.value)}><option value="">{modelOptions.length ? "All models" : "No admin models"}</option>{modelOptions.map(item => <option value={item} key={item}>{item}</option>)}</select></label><label>From<DateInput value={filters.date_from} max={filters.date_to || undefined} onChange={value => setFilter("date_from", value)} ariaLabel="From date, DD/MM/YYYY" /></label><label>To<DateInput value={filters.date_to} min={filters.date_from || undefined} onChange={value => setFilter("date_to", value)} ariaLabel="To date, DD/MM/YYYY" /></label><button className="filter" onClick={() => setFilters({ ...emptyFilters })}>Clear</button></section>
