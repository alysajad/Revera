...
import { getLeadDetail, getManagerLeads, sourceName, statusName, type Lead, type LeadDetail } from "@/lib/crm";
...
export function ManagerLeadsPage() {
  ...
  return <section className="page manager-page manager-leads-page">
    ...
    <section className="panel manager-filter-bar manager-lead-filters"><label>Search<input value={filters.q} onChange={event => setFilter("q", event.target.value)} placeholder="Name, phone, model" /></label><label>Source<select value={filters.source} onChange={event => setFilter("source", event.target.value)}><option value="">All sources</option>{["META", "WEBSITE", "CARWALE", "WALKIN", "CAMPAIGN", "OTHER", "UNKNOWN"].map(item => <option value={item} key={item}>{sourceName(item)}</option>)}</select></label><label>Status<select value={filters.status} onChange={event => setFilter("status", event.target.value)}><option value="">All statuses</option>{["FRESH", "RNR", "SWITCHED_OFF", "CALLBACK", "PENDING", "QUALIFIED", "WALKIN", "WON", "LOST", "UNQUALIFIED"].map(item => <option value={item} key={item}>{statusName(item)}</option>)}</select></label><label>Category<select value={filters.category} onChange={event => setFilter("category", event.target.value)}><option value="">All categories</option><option value="HOT">Hot</option><option value="WARM">Warm</option><option value="COLD">Cold</option></select></label><label>Model<input value={filters.model} onChange={event => setFilter("model", event.target.value)} placeholder="Model" /></label><label>From<input value={filters.date_from} onChange={event => setFilter("date_from", event.target.value)} placeholder="DD/MM/YYYY" /></label><label>To<input value={filters.date_to} onChange={event => setFilter("date_to", event.target.value)} placeholder="DD/MM/YYYY" /></label><button className="filter" onClick={() => setFilters({ q: "", source: "", status: "", model: "", category: "", cre: "", ps: "", date_from: "", date_to: "" })}>Clear</button></section>
