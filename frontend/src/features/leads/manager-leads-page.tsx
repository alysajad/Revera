...
const dateInputValue = (value: string) => String(toApiDate(value) || "");
const displayDateValue = (value: string) => formatDate(value);

function initialFilters() {
  if (typeof window === "undefined") return { q: "", source: "", status: "", model: "", category: "", cre: "", ps: "", date_from: "", date_to: "", range: "", sales_outcome: "", flagged: "", followup: "", risk: "", status_group: "" };
...
export function ManagerLeadsPage() {
  ...
  const query = useMemo(() => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (!value) return;
      params.set(key, key.startsWith("date_") ? String(toApiDate(value) || value) : value);
    });
  ...
  return <section className="page manager-page manager-leads-page">
    ...
    <section className="panel manager-filter-bar manager-lead-filters"><label>Search<input value={filters.q} onChange={event => setFilter("q", event.target.value)} placeholder="Name, phone, model" /></label><label>Source<select value={filters.source} onChange={event => setFilter("source", event.target.value)}><option value="">All sources</option>{["META", "WEBSITE", "CARWALE", "WALKIN", "CAMPAIGN", "OTHER", "UNKNOWN"].map(item => <option value={item} key={item}>{sourceName(item)}</option>)}</select></label><label>Status<select value={filters.status} onChange={event => setFilter("status", event.target.value)}><option value="">All statuses</option>{["FRESH", "RNR", "SWITCHED_OFF", "CALLBACK", "PENDING", "QUALIFIED", "WALKIN", "WON", "LOST", "UNQUALIFIED"].map(item => <option value={item} key={item}>{statusName(item)}</option>)}</select></label><label>Category<select value={filters.category} onChange={event => setFilter("category", event.target.value)}><option value="">All categories</option><option value="HOT">Hot</option><option value="WARM">Warm</option><option value="COLD">Cold</option></select></label><label>Model<select value={filters.model} onChange={event => setFilter("model", event.target.value)}><option value="">{modelOptions.length ? "All models" : "No admin models"}</option>{modelOptions.map(item => <option value={item} key={item}>{item}</option>)}</select></label><label>From<input type="date" lang="en-GB" value={dateInputValue(filters.date_from)} onChange={event => setFilter("date_from", displayDateValue(event.target.value))} aria-label="From date, DD/MM/YYYY IST" /></label><label>To<input type="date" lang="en-GB" value={dateInputValue(filters.date_to)} onChange={event => setFilter("date_to", displayDateValue(event.target.value))} aria-label="To date, DD/MM/YYYY IST" /></label><button className="filter" onClick={() => setFilters({ q: "", source: "", status: "", model: "", category: "", cre: "", ps: "", date_from: "", date_to: "", range: "", sales_outcome: "", flagged: "", followup: "", risk: "", status_group: "" })}>Clear</button></section>
