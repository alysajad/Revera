...
export function LeadDesk({ officerMode = false, followUpsOnly = false }: { officerMode?: boolean; followUpsOnly?: boolean }) {
  ...
  return <section className="page">
    <div className="page-heading compact"><div><p className="eyebrow">{heading.toUpperCase()}</p><h1>{officerMode ? <>Keep the <span>promise.</span></> : <>Move leads to the <span>right rider.</span></>}</h1><p className="subtext">{officerMode ? "Your assigned conversations and follow-ups." : "Assign fresh leads to CRE, then hand qualified leads to PS/SO."}</p></div>{!officerMode && assignmentView === "fresh" && <button className="button primary" onClick={autoAssign} disabled={!leads.length}>↻ Auto assign {leads.length} leads</button>}</div>
    {!officerMode && <nav className="sales-tabs" aria-label="Assignment mode"><button className={assignmentView === "fresh" ? "active" : ""} onClick={() => { setAssignmentView("fresh"); setBulkOfficerId(""); setPage(1); }}>Fresh to CRE</button><button className={assignmentView === "qualified" ? "active" : ""} onClick={() => { setAssignmentView("qualified"); setBulkOfficerId(""); setPage(1); }}>Qualified to PS/SO</button></nav>}
